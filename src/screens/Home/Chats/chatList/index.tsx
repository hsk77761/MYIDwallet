...
export const ChatList = () => {
  ...
  const [List, setChatsList] = useState<IUsers[]>([])
  ...
  useEffect(() => {
    socket.on(currentAccount?.address || "", (res: IUsers) => {
      if (currentAccount?.address !== res.user) {
  ...
  const getInitialData = async (loading: boolean) => {
    setLoading(loading)
    axiosInstance.get(`/chats/${currentAccount?.address}?${Date.now()}`).then((response) => {
          ...
          <>
            <ScrollView
              ...
              {loading ? <ActivityLoading /> :
                <>
                  {List && List.length && List.length !== 0 ? List.map((user: IUsers, i: number) => {
                    return (<>
                      <TouchableOpacity
                        key={i}
                        onPress={() => {
